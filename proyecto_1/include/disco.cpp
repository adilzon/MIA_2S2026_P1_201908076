...
{ 
    ...
    string path = "";
    bool error = false;
    for (string token:tokens) //[size=10, u=m, path=/home/hola.dk path=/home/hola.dk]
    {
    ...
    if (u.empty())
    {
        u = "M"; //pordefeto en megas U=k | u=m |u=Giga
    }
    if (error)
    {
        return;
    ...
    {
        scan.errores("MKDISK ", " Obligatorio el parametro Path y Size");
    }else if(path.empty()){
        scan.errores("MKDISK "," Obligatorio el parametro Path ");
    }else if (size.empty())
    {
        scan.errores("MKDISK"," Obligatorio el parametro Size para este comando");
    ...
    {
        scan.errores("MKDISK","valores de parametro U no vaidos");
    }
    else{
...
void Disk::makeDisk(string s, string f, string u, string path){
    ...
    try
    {
        ...
        if (validar != NULL)
        {
            scan.errores("MKDISK", " Error: Disco ya existente");
            fclose(validar);
            return;
        }
        ...
        try
        {
            ...
            if(imprimir!=NULL){
                ...
                scan.respuesta("MKDISK"," PATH:  \033[97;48;5;208m"+path2+"\033[0m");
                cout<< endl;
                cout << "\033[30;42mPARAMETROS: \033[0m" << endl;
                cout << "\033[97;48;5;208mSize:  \033[0m"<< discoI.mbr_tamano << endl;
